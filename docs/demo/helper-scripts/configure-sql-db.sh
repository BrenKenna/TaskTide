...
select ItemState, COUNT(DISTINCT id) AS 'Total', SUM(TaskCount),
    SUM(TaskDone) FROM WorkItem GROUP BY ItemState ORDER BY 'Total' DESC;
...
select ItemState, COUNT(DISTINCT id) AS 'Total', SUM(TaskCount), SUM(TaskDone) 
    FROM WorkItem GROUP BY ItemState ORDER BY 'Total' DESC;
