 ...
 */
public class FileChannelActor extends IntraProcessActor {
    ...
    public synchronized boolean release(Mutex mutex) throws MutexCheckedException {
        ...
        if ( active == null ) {
            throw new MutexCheckedException("No mutex file channel mutex");
        }
        
        if ( !active.getId().equals(mutex.getId()) ) {
            throw new MutexCheckedException("Active mutex does not match provided");
        }
    ...
    public synchronized boolean release(Path targetFile) throws MutexCheckedException {
        if ( active == null ) {
            throw new MutexCheckedException("No active mutex to release");
        }
